...
from core.model.utils import get_same_padding, sub_filter_start_end, make_divisible
from core.model.common import SEModule
    ...
    def __init__(self, max_in_channels, kernel_size_list, stride=1, dilation=1):
        ...
        if self.KERNEL_TRANSFORM_MODE is not None:
            ...
            for i in range(len(self._ks_set) - 1):
                ...
                # print(f"register parameter {param_name}")
    ...
    def get_active_filter(self, in_channel, kernel_size):
        ...
        if self.KERNEL_TRANSFORM_MODE is not None and kernel_size < max_kernel_size:
            ...
            for i in range(len(self._ks_set) - 1, 0, -1):
                # print(start_filter.abs().sum(dim=(1,2,3)))
                src_ks = self._ks_set[i]
                if src_ks <= kernel_size:
                    break
                target_ks = self._ks_set[i - 1]
                # print(f"transform {src_ks} kernel to {target_ks}")
