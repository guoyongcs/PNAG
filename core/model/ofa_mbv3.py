...
from core.model.dynamic.layers import DynamicMBConvLayer, DynamicConvLayer, DynamicLinearLayer
# from elastic_nn.modules.dynamic_layers import DynamicMBConvLayer, DynamicConvLayer, DynamicLinearLayer
from core.model.layers import ConvLayer, IdentityLayer, LinearLayer, MBInvertedConvLayer
from core.model.mobilenet_v3 import MobileNetV3, MobileInvertedResidualBlock
# from .utils import make_divisible, int2list
# from core.model.layer import ConvLayer, IdentityLayer, LinearLayer, MBInvertedConvLayer
from core.model.utils import make_divisible, int2list
...
class OFAMobileNetV3(MobileNetV3):
    ...
    def __init__(self, n_classes=1000, bn_param=(0.1, 1e-5), dropout_rate=0.1, base_stage_width=None,
        ...
        if len(set(input_channel)) == 1:
            ...
            first_block_conv = MBInvertedConvLayer(
                in_channels=max(input_channel), out_channels=max(input_channel), kernel_size=3,
        ...
        if len(final_expand_width) == 1:
            final_expand_layer = ConvLayer(max(feature_dim), max(final_expand_width),
                                           kernel_size=1, act_func='h_swish')
            feature_mix_layer = ConvLayer(
                max(final_expand_width), max(last_channel), kernel_size=1, bias=False,
    ...
    def re_organize_middle_weights(self, expand_ratio_stage=0):
        for i, block in enumerate(self.blocks[1:]):
            rev = block.mobile_inverted_conv.re_organize_middle_weights(expand_ratio_stage)
            if rev is not None:
                print(i)
                raise ValueError()
